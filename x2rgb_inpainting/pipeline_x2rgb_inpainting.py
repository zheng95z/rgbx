...
class StableDiffusionAOVDropoutPipeline(
    ...
    def __call__(
        ...
        masked_image = self.image_processor.preprocess(masked_image)
        photo = self.image_processor.preprocess(photo)
            ...
            if aov_name == "normal":
                if normal is not None:
                    preprocessed_aovs[aov_name] = (
                        self.image_processor.preprocess_normal(normal * (1 - mask))
                    )
                else:
                    preprocessed_aovs[aov_name] = None
        ...
        mask_latents = F.interpolate(
            mask.to(device=device, dtype=prompt_embeds.dtype).unsqueeze(0),
            size=(height_latent, width_latent),
            mode="bilinear",
            align_corners=False,
            antialias=True,
        ...
        if do_classifier_free_guidance:
            uncond_mask_latent = torch.zeros_like(mask_latents)
            mask_latents = torch.cat(
                [mask_latents, mask_latents, uncond_mask_latent], dim=0
            )
        image_latents.append(masked_image_latents)
        image_latents.append(mask_latents)
        ...
        masked_image = self.vae.decode(
            masked_image_latents[0:1] / self.vae.config.scaling_factor,
            return_dict=False,
        )[0]
        masked_image = self.image_processor.postprocess(
            masked_image, output_type=output_type, do_denormalize=do_denormalize
        )

        photo = self.vae.decode(
            photo_latents[0:1] / self.vae.config.scaling_factor, return_dict=False
        )[0]
        photo = self.image_processor.postprocess(
            photo, output_type=output_type, do_denormalize=do_denormalize
        )
        image = [image, masked_image, photo]
